 ...
 * Return: IRQ_HANDLED
 */
static irqreturn_t arbif_xenfront_xen_event_handler(int irq, void *dev)
{
	...
	struct xengpu_sring **sring)
{
	if ((*grantref) != INVALID_GRANT_REF) {
		...
		gnttab_end_foreign_access(*grantref, (struct page *)(*sring));
#elif (KERNEL_VERSION(5, 18, 0) <= LINUX_VERSION_CODE)
		gnttab_end_foreign_access(*grantref, (unsigned long)(*sring));
#else
...
#else
	err = xenbus_setup_ring(xfdev->xdev, GFP_KERNEL | __GFP_ZERO, (void **)&sring,
				NUM_PAGES_PER_RING, &gref);
#endif
	if (err < 0) {
...
{
	dev_dbg(&xfdev->xdev->dev, "freeing xfdev @ 0x%pK\n", xfdev);
