...
static void on_gpu_lost(struct mali_vm_priv_data *vm)
{
	...
	/* Even in PV we need to unassign the VM in case a GPU Lost happens to
	 * avoid the possibility of sending a LOST message while GPU is still
	 * available.
	 */
	assign_vm_gpu(dev, VM_UNASSIGNED);
