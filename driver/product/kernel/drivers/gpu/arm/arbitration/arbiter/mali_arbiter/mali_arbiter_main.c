...
MODULE_PARM_DESC(request_timeout, "GPU time in ms given before switching to another VM");
...
MODULE_PARM_DESC(slice_power_off_wait_time, "Max time(ms) to wait before powering off slices");
				...
				u32 *old_slices)
{
	...
	*old_slices = partition->slices;
	...
	if (!cfg_ops || !cfg_ops->assign_slices) {
		dev_err(arb->dev, "P%u: Repartition interface not available.\n", partition->index);
		ret = -ENODEV;
		goto exit;
	}
...
static void get_utilisation(void *dev, u32 *gpu_busytime, u32 *gpu_totaltime)
{
	...
	*gpu_totaltime = 0;
	...
	for (i = 0; i < ARRAY_SIZE(arb->partitions); ++i) {
		...
		dev_dbg(arb->dev, "Arbiter_%d P%u reporting busy %u, total %u (~ %llu%%)\n",
			arb->arb_id, partition->index, busytime, totaltime, percent);
...
static int arbiter_get_pwr(struct mali_arb *arb)
{
	...
	if (!pwr_node) {
		dev_info(arb->dev, "No GPU Power in Arbiter Device Tree Node\n");
...
static int arbiter_init_pwr(struct mali_arb *arb)
{
	...
	if (arb->pwr_if.ops->register_arb == NULL || arb->pwr_if.ops->unregister_arb == NULL) {
	 ...
	 */
	err = arb->pwr_if.ops->register_arb(arb->pwr_if.dev, arb, &ops, &arb->arb_id);
	if (err) {
		dev_err(arb->dev, "Failed to register arbiter with GPU power device\n");
		return -err;
	}
...
static struct mali_arb_partition *build_partition(struct mali_arb *arb, u32 index)
{
	...
	if (!partition) {
		dev_err(arb->dev, "P%u: Failed to allocate partition memory\n", index);
		return NULL;
	}
		   ...
		   struct mali_arb_data **arb_data)
{
	...
	if (pwr.dev) {
		...
		if (err) {
			dev_err(arb->dev, "Failed to initialize GPU Power interfaces.\n");
			goto cleanup;
		}
	}
...
static int arbiter_probe(struct platform_device *pdev)
{
	...
	if (arb->pwr_if.dev) {
		err = arbiter_init_pwr(arb);
		if (err) {
			dev_err(arb->dev, "Failed to initialize GPU Power device\n");
			goto cleanup_module;
		}
	}

	if (no_timeslice_yield_timeout < yield_timeout)
		dev_warn(arb->dev, "no_timeslicing_yield_timeout is shorter than yield_timeout");
 ...
 * resources.
 */
static void term_arbiter(struct mali_arb_data *arb_data, struct device *dev)
{
	struct mali_arb_partition *partition;
	struct mali_arb *arb;
	uint32_t i;
